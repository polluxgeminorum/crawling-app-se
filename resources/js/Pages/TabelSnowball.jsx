...
export default function TabelSnowball() {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('/api/snowball', {
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            await axios.put(`/api/snowball/${selectedItem.id}`, editForm, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleDelete = async () => {
        setIsSubmitting(true);
        try {
            await axios.delete(`/api/snowball/${selectedItem.id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleSync = async () => {
        setIsSyncing(true);
        try {
            const response = await axios.post('/api/snowball/sync', {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
            showNotification('success', response.data.message);
            fetchData(); // Refresh data after sync
        } catch (err) {
            showNotification('error', err.response?.data?.message || 'Gagal melakukan sinkronisasi');
        } finally {
            setIsSyncing(false);
        }
    };
    ...
    const exportToExcel = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'Data Snowball');
        XLSX.writeFile(workbook, 'data_snowball.xlsx');
    };

    return (
        <SidebarLayout title="Tabel Data Snowball">
            {/* Toast Notification */}
            {notification && (
                ...
                <div className="fixed inset-0 bg-white/30 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl shadow-2xl border-2 border-blue-200 w-full max-w-lg mx-4">
                        <div className="p-6 border-b">
                            <h3 className="text-xl font-bold">Edit Data Snowball</h3>
                        </div>
                        <form onSubmit={handleUpdate} className="p-6 space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">Nama Pelaku Usaha / Keluarga</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">Nama Pengisi / Kontak</label>
                                <input
                                    type="text"
                                    value={editForm.nama_pengisi}
                                    onChange={(e) => setEditForm({...editForm, nama_pengisi: e.target.value})}
                                    className="w-full px-4 py-2 rounded-lg border border-slate-300 focus:border-blue-500"
                                    required
                                />
                            </div>
                            ...
                            <table className="w-full text-sm text-left">
                                <thead className="text-xs text-slate-700 uppercase bg-slate-50">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 cursor-pointer hover:bg-slate-100" onClick={() => handleSort('nama_pengisi')}>
                                            Nama Pengisi {getSortIcon('nama_pengisi')}
                                        </th>
